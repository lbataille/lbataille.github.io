function [paramPERC,curvePERC,dT,dFCOVERMC] = applyMontecarlo(resultsP,GF,pheno,nRep)
...
%     pheno = [];
    ...
    for i = 1:nDays
        dFCOVERMC(:,i) = unifrnd(dFCOVER01(i),dFCOVER99(i),[nRep,1]);
    end
...
%     pheno.DNUM = datenum(resultsP.DT);
    ...
    for k = 1:nRep
        ...
        FCOVERk = dFCOVERMC(k,:);
        [DTk,dFCOVERk,isNFilled] = padZeros(dT,FCOVERk,pheno,20);
        gddk = interp1(datenum(pheno.date),pheno.GDDc,datenum(DTk));   
        gddk(isnan(gddk)) = linspace(3000,5000,nnz(isnan(gddk)));
        w = ones(size(gddk));
    ...
    paramPERC.perc = perc';
    FCOVERu = prctile(dFCOVERMC1,perc,1);
